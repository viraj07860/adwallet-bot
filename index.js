...
async function createWithdrawFeeInvoice() {
  return bot.telegram.createInvoiceLink(
    'Withdrawal Fee',
    ...
    '',
    'XTR',
    [{ label: 'Withdrawal Fee', amount: WITHDRAW_FEE }]
  );
}
...
app.get('/api/withdraw-fee-invoice', async (req, res) => {
  try {
    const invoiceUrl = await createWithdrawFeeInvoice();
    return res.json({ ok: true, invoiceUrl });
  } catch (err) {
    console.error('Withdraw fee invoice error:', err);
    return res.status(500).json({
      ok: false,
      error: err.message || 'Could not create fee invoice'
    });
  }
});
...
app.post('/api/withdraw', async (req, res) => {
  try {
    ...
    if (!user.feePaid) {
      return res.json({
        success: false,
        message: 'Withdrawal fee not approved yet.'
      });
    }
...
bot.command('buyfee', async (ctx) => {
  try {
    const invoiceUrl = await createWithdrawFeeInvoice();
    return ctx.reply('Pay the withdrawal fee with Telegram Stars:', {
      reply_markup: {
        inline_keyboard: [[
          { text: `Pay ${WITHDRAW_FEE} Stars`, url: invoiceUrl }
        ]]
      }
    });
  } catch (err) {
    console.error('bot.command buyfee error:', err);
    return ctx.reply('❌ Could not create fee invoice');
  }
});

bot.command('feeproof', async (ctx) => {
  try {
    const userId = String(ctx.from.id);
    const user = await ensureUser(userId, ctx.from.username || ctx.from.first_name || 'User');

    user.feeProofPending = true;
    await user.save();

    return ctx.reply('📸 Send your withdrawal fee screenshot now.');
  } catch (err) {
    console.error('bot.command feeproof error:', err);
    return ctx.reply('❌ Could not start fee proof flow.');
  }
});

bot.command('approvefee', async (ctx) => {
  try {
    const senderId = String(ctx.from.id);
    if (senderId !== ADMIN_ID) {
      return ctx.reply('❌ Admin only');
    ...
    if (!targetUserId) {
      return ctx.reply('Usage: /approvefee <userId>');
    }

    const user = await ensureUser(targetUserId);
    user.feePaid = true;
    user.feeProofPending = false;
    await user.save();

    await ctx.reply(`✅ Fee approved for ${targetUserId}`);

    try {
      await bot.telegram.sendMessage(
        targetUserId,
        '✅ Your withdrawal fee has been approved. You can now request withdrawal.'
      );
    } catch {}
  } catch (err) {
    console.error('bot.command approvefee error:', err);
  }
});

bot.command('rejectfee', async (ctx) => {
  try {
    const senderId = String(ctx.from.id);
    if (senderId !== ADMIN_ID) {
      return ctx.reply('❌ Admin only');
    ...
    if (!targetUserId) {
      return ctx.reply('Usage: /rejectfee <userId>');
    }
    ...
    await ctx.reply(`❌ Fee rejected for ${targetUserId}`);

    try {
      await bot.telegram.sendMessage(
        targetUserId,
        '❌ Your withdrawal fee proof was rejected. Please send a correct screenshot.'
      );
    } catch {}
  } catch (err) {
    console.error('bot.command rejectfee error:', err);
  }
});
...
bot.on('photo', async (ctx) => {
  try {
    ...
    if (user.feeProofPending) {
      user.feeProofPending = false;
      await user.save();

      await ctx.reply('✅ Screenshot received. Waiting for admin approval.');
    ...
    if (user.isWaitingForProof) {
      ...
      await ctx.reply('✅ VIP screenshot received! Admin will review it soon.');
...
bot.on('message', async (ctx) => {
  try {
    ...
    if (payload === 'withdraw_fee') {
      user.feePaid = true;
      await user.save();
      await ctx.reply('✅ Withdrawal fee payment received. Withdrawal is now unlocked.');
      return;
    }
