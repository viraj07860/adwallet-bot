...
async function handleReward(req, res, payload) {
  try {
    ...
    const fingerprint = payload.fingerprint ? String(payload.fingerprint) : '';
...
async function sendVipInvoice(ctx, plan) {
...
bot.command('buyvip', async (ctx) => {
  const keyboard = Object.keys(VIP_STARS).map(plan => ([
    { text: `⭐ Buy ${plan} (${VIP_STARS[plan]} Stars)`, callback_data: `vipplan_${plan}` }
  ]));

  await ctx.reply('Choose a VIP plan:', {
...
bot.action(/^vipplan_(.+)$/, async (ctx) => {
  try {
    ...
    await sendVipInvoice(ctx, plan);
  } catch (err) {
    console.error('VIP invoice error:', err);
    await ctx.answerCbQuery('Could not open payment', { show_alert: true });
  }
});
